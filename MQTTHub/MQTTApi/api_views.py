...
class DevicesApiForUserView(APIView):
    ...
    def get_all(self, request, format=None):
        ...
        if me['is_staff']:
            ...
            user_permissions = AuthServiceApi.get_user_permissions(HUB_ID, me['pk'])
            group_permissions = AuthServiceApi.get_group_permissions(HUB_ID, list(map(lambda group: group['pk'], me['groups'])))
            devices_pk = set(
